...
Hooks:PostHook(CoreWorldInstanceManager, "_get_instance_mission_data", "eclipse__get_instance_mission_data", function(self, path)
	...
	if func then
		...
		if result then
			...
			Eclipse:warn_console("Instance %s unavailable!", path)
		end
	end
end)
