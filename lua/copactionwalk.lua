...
-- Rotation speed multiplier, epic function overrides
function CopActionWalk:update(t)
	...
	if move_dir and not self._expired then
		...
		if self._curve_path_end_rot then
			...
			local rot_mul = self._common_data.char_tweak.rotation_speed or 1
			...
			mrot_slerp(rot_new, self._common_data.rot, rot_new, math.min(1, dt * 5 * rot_mul))
		end
...
function CopActionWalk:_upd_start_anim(t)
	...
	if self._start_run_turn then
		...
		if not self._end_of_curved_path then
			...
			mrot_lookat(temp_rot1, wanted_u_fwd, math.UP)
			mrot_slerp(temp_rot1, self._common_data.rot, temp_rot1, math.min(1, dt * 5 * rot_mul))
...
function CopActionWalk:_upd_stop_anim(t)
	local dt = TimerManager:game():delta_time()
	local rot_mul = self._common_data.char_tweak.rotation_speed or 1
	local rot_new = self._common_data.rot:slerp(Rotation(self._stop_anim_fwd, math.UP), math.min(1, dt * 5 * rot_mul))
...
Hooks:PreHook(CopActionWalk, "init", "eclipse_init", function(self, action_desc, common_data)
