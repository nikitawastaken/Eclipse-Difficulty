...
function GroupAIStateBase:spawn_one_teamAI(is_drop_in, char_name, pos, rotation, start)
	if
		not managers.groupai:state():team_ai_enabled()
		or not self._ai_enabled
		or not managers.criminals:character_taken_by_name(char_name) and managers.criminals.MAX_NR_TEAM_AI <= managers.criminals:nr_AI_criminals()
	then
		return
	end
	...
	if objective and objective.type == "follow" then
		...
		log("VISIBILITY TAG: ", tostring(unit_name), tostring(spawn_pos), tostring(spawn_rot))
		...
		managers.network
			...
			:send_to_peers_synched(
				...
				unit,
				character_name,
				managers.blackmarket:henchman_loadout_string_from_loadout(loadout),
				0,
				0,
