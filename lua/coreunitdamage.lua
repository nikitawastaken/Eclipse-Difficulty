...
Hooks:PostHook(CoreBodyDamage, "init", "eclipse_init", function(self)
	...
	if self._unit:character_damage() and self._unit:character_damage().IS_TANK then
		...
		local dmg_mul = tweak_data.character[self._unit:base()._tweak_table].tank_armor_health_mul

		if not dmg_mul then
			-- nothing
		elseif self._body_element._name:find("glass") then
			local glass_dmg_mul = dmg_mul * 2

			self._body_element._damage_multiplier = (glass_dmg_mul / tank_balance_mul) or self._body_element._damage_multiplier
		else
			local armor_dmg_mul = dmg_mul

			self._body_element._damage_multiplier = (armor_dmg_mul / tank_balance_mul) or self._body_element._damage_multiplier
		end
	elseif
