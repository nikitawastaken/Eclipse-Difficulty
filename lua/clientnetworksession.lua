...
Hooks:PostHook(ClientNetworkSession, "notify_host_when_outfits_loaded", "eclipse_post_client_load", function()
	if Eclipse.current_environment and (env ~= Eclipse.current_environment or grading ~= Eclipse.color_grading) then
		...
		env = Eclipse.current_environment
		grading = Eclipse.color_grading
	end
end)
