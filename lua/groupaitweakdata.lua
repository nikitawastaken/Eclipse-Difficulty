...
Hooks:PostHook(GroupAITweakData, "_init_enemy_spawn_groups", "eclipse__init_enemy_spawn_groups", function(self, difficulty_index)
	...
	self.enemy_spawn_groups.cs_swats = {
		amount = small_squads and { 2, 3 } or { 3, 4 },
		spawn = {
			{
				freq_by_diff = { 3, 0.75, 0 },
				amount_max = full_force and 0 or 3,
				rank = 2,
				unit = "cs_swat",
	...
	self.enemy_spawn_groups.cs_heavies = {
		...
		spawn = {
			...
			},
			{
				...
				amount_max = heavy_response and 0 or 2,
				rank = 1,
				unit = "cs_swat_1_3",
	...
	self.enemy_spawn_groups.cs_shield = {
		amount = small_squads and { 3, 3 } or { 4, 4 },
		spawn = {
			{
				freq = 1,
				amount_min = 1,
			...
			},
			{
				...
				amount_min = heavy_response and 2 or 0,
				rank = 2,
				unit = "cs_heavy",
				tactics = self._tactics.shield_support,
			},
	...
	self.enemy_spawn_groups.fbi_shield = {
		...
		spawn = {
			{
				freq = (difficulty_index ^ 2) / 75 * (special_operation and 1.5 or 1),
				amount_min = 1,
				amount_max = small_squads and 1 or 2,
				rank = 3,
				unit = "fbi_shield",
				random_tactics = self._random_tactics.shield,
			},
	...
	self.enemy_spawn_groups.fbi_bulldozer = {
		...
		spawn = {
			{
				freq = (difficulty_index ^ 2) / 200 * (special_operation and 1.5 or 1),
				amount_min = 1,
				amount_max = small_squads and 1 or 2,
				rank = 3,
				unit = "bulldozer",
...
function GroupAITweakData:_init_enemy_spawn_groups_level(tweak_data, difficulty_index)
	...
	if self.us_army_heists[level_id] then
		self.timed_enemy_spawn_groups.us_group1 = Eclipse:require("timed_groups/us_group1")(self._timed_tactics, self._tactics)
	end
	...
	if self.us_army_heists_scripted[level_id] then
		self.timed_enemy_spawn_groups.us_scripted_group1 = Eclipse:require("timed_groups/us_scripted_group1")(self._timed_tactics, self._tactics)
	end
end
	...
	for _, group_ai_state_name in pairs({ "besiege", "street", "safehouse", "ponr", "skirmish" }) do
		...
		if assault_state then
			...
			if assault_state.reenforce then
				assault_state.recon.force = table_multiplier(assault_state.reenforce.interval, level_settings.reenforce_interval_mul or 1)
...
Hooks:PostHook(GroupAITweakData, "_init_task_data", "eclipse__init_task_data", function(self, difficulty_index)
	...
	elseif difficulty_index == 5 then
		...
		self.besiege.reenforce.groups = {
			fbi_defend_init = { 1, 1, 0 },
			fbi_defend_light = { 0, 1, 0 },
			fbi_defend_heavy = { 0, 0, 1 },
			elite_defend_light = { 0, 0, 1 },
		}
	end
	...
	elseif difficulty_index == 5 then
		self.ponr.assault.groups = {
			...
			elite_shield = { 10, 10, 10 },

			elite_taser = { 20, 20, 20 },
		...
		self.ponr.assault.groups = {
			...
			fbi_heavies = { 40, 40, 40 },

			elite_shield = { 24, 24, 24 },

			elite_taser = { 24, 24, 24 },

			fbi_cloaker = { 24, 24, 24 },

			elite_bulldozer = { 12, 12, 12 },
		}
		self.ponr.reenforce.groups = {
			elite_defend_light = { 1, 1, 1 },
