	...
	for _, task_data in pairs(undershot_tasks) do
		...
		elseif self._task_data.reenforce.next_dispatch_t < self._t then
			...
			if spawned then
				...
				self._task_data.reenforce.next_dispatch_t = self._t + math.max(tweak_data.group_ai.min_reenforce_interval, self:_get_difficulty_dependent_value(self._tweak_data.reenforce.interval) / #undershot_tasks)
				break
			end
		else
			break
