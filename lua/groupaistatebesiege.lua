		...
		elseif self._task_data.reenforce.next_dispatch_t < self._t then
			...
			if spawned then
				...
				self._task_data.reenforce.next_dispatch_t = self._t
					+ math.max(min_reenforce_interval, self:_get_difficulty_dependent_value(self._tweak_data.reenforce.interval) - #undershot_tasks * tweak_data.group_ai.undershot_reenforce_mul)
				break
			end
		else
			break
